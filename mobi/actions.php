...
/*if($action == 'upload'){
	$path = $_GET['path'];
	$new_name = $_POST['give_name'];
	if (post_exist($my_id, $new_name, $path) == false){
			...
			if(test_img($extension)){
				if (($_FILES[$file]["size"] < 3*MB)){
					if ($_FILES[$file]["error"] > 0){
						...
						$place = "uploads/".$path."/".$my_id."/images/" . $my_id.".".$new_name.".".$extension;
						
						move_uploaded_file($_FILES[$file]["tmp_name"], $place);
						mysql_query("INSERT INTO `xpoliem`.`$path` (`post_id` ,`user_id` ,`title`,`category_id`,`post_date`,`post_exp_date`)VALUES('', '$my_id', '$new_name','','now()','')");
						mysql_query("UPDATE `xpoliem`.`$path` SET `post_extension`='$extension' WHERE `user_id`='$my_id' AND `title`='$new_name'");
						$created = mysql_fetch_assoc(mysql_query("SELECT `post_id`,`post_date` FROM `xpoliem`.`$path` WHERE `title` = '$new_name' AND `user_id` = '$my_id'"));
					...
					header('location: post_form.php?u_id='.$u_id.'&to=lib&MB=3');
				}
			}else{header('location: post_form.php?u_id='.$u_id.'&to=lib&fault_inp=recognised image');}
		}else{
			if($file_type == 'video'){
				if(test_vid($extension)){
					...
					if (($_FILES[$file]["size"] < 30*MB)){
						if ($_FILES[$file]["error"] > 0){
							...
							$place = "uploads/".$path."/".$my_id."/videos/" . $my_id.".".$new_name.".".$extension;
								//ffmpeg
						
								$ffmpeg = 'C:\\ffmpeg\\bin\\ffmpeg';
								$imageFile = 'uploads/lib/'.$my_id.'/videos/thumbs/'.$new_name.'.jpg';
								$size = '120x90';
								$getFromSecond = 5;
							...
							$video = new Video(new \SplFileInfo($place));
							$nw_name = $_FILES[$file] = $video->encodeInto("ogv");
							mysql_query("INSERT INTO `xpoliem`.`$path` (`post_id` ,`user_id` ,`title`,`category_id`,`post_date`,`post_exp_date`)VALUES('', '$my_id', '$new_name','','now()','')");
							mysql_query("UPDATE `xpoliem`.`$path` SET `post_extension`='$extension' WHERE `user_id`='$my_id' AND `title`='$new_name'");
							$created = mysql_fetch_assoc(mysql_query("SELECT `post_id`,`post_date` FROM `xpoliem`.`$path` WHERE `title` = '$new_name' AND `user_id` = '$my_id'"));
							$post_id = $created['post_id'];
						...
						header('location: post_form.php?u_id='.$u_id.'&to=lib&MB=30');
					}
				}else{
					header('location: post_form.php?u_id='.$u_id.'&to=lib&fault_inp=recognised video');
				}
			}
		}
		header('location: post_form.php?u_id='.$u_id.'&to=lib');
	}else{
		header('location: post_form.php?u_id='.$u_id.'&to=lib&exists=true&name='.$new_name);
	}
