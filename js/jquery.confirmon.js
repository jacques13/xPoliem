...
(function($) {
    ...
    confirmOn.createBox = function($element) {
		...
		var title = $element.data('confirmon').options.title;
        var date = $element.data('confirmon').options.date;
		var TopPosition = $element.data('confirmon').options.TopPosition;

        var $box = $('<div/>').addClass(classPrepend + '-box').css({ top:TopPosition }).hide().appendTo('body');
        $('<h3/>')
            .html(title)
            .appendTo($box);
			$('<h4/>')
