...
if($action == 'view'){
	$views = mysql_fetch_assoc(mysql_query("SELECT `views`FROM `xpoliem`.`posts` WHERE `post_id` = '$post_id' "));
	$views = $views['views'];
	if ($views == NULL){$views = 0;}
	$views++;
	mysql_query("UPDATE `xpoliem`.`posts` SET `views`='$views' WHERE `post_id`='$post_id'");
}
